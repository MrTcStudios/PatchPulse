...
.wrap { max-width: 720px; margin: 0 auto; padding: 0 clamp(1.5rem, 5vw, 3rem) clamp(3rem, 8vw, 5rem); }
...
#emailInput { width: 100%; background: rgba(255,255,255,0.04); border: 1px solid var(--line); border-radius: 0; color: var(--ink); font-family: var(--sans); font-size: 1rem; padding: .9rem 1.1rem; transition: border-color .2s, box-shadow .2s; }
#emailInput::placeholder { color: var(--mut); }
#emailInput:focus { outline: none; border-color: var(--violet); box-shadow: 0 0 0 3px rgba(157,134,255,.18); }
.cf-turnstile { margin-top: 12px; }
.cf-turnstile iframe { max-width: 100% !important; }
.check-button { display: inline-flex; align-items: center; justify-content: center; font-family: var(--sans); font-size: .95rem; font-weight: 600; color: #14121f; background: var(--violet); border: none; border-radius: 0; padding: .95rem 1.7rem; cursor: pointer; transition: background .2s, transform .15s; }
.check-button:hover { background: var(--violet-2); }
.check-button:active { transform: translateY(1px); }
.check-button:disabled { opacity: .6; cursor: default; }

/* loading */
.loading { display: none; align-items: center; justify-content: flex-start; gap: .8rem; padding: 1.6rem 0; color: var(--mut); font-size: .95rem; }
.spinner { width: 20px; height: 20px; border: 2px solid rgba(255,255,255,.15); border-top-color: var(--violet); border-radius: 50%; animation: spin .8s linear infinite; flex-shrink: 0; }
@keyframes spin { to { transform: rotate(360deg); } }

/* risultati (HTML iniettato da script.js) */
.results { margin-top: 1.9rem; }
.breach-count { font-family: var(--disp); font-weight: 500; font-size: clamp(1.6rem, 3.5vw, 2.3rem); line-height: 1.1; letter-spacing: -.015em; margin-bottom: .9rem; display: flex; align-items: center; gap: .55rem; flex-wrap: wrap; }
.results.safe   .breach-count { color: var(--ok); }
.results.danger .breach-count { color: var(--red); }
.results.error  .breach-count { color: var(--red); }
.results > p { color: var(--ink-2); font-size: 1rem; line-height: 1.65; margin-bottom: 1.1rem; max-width: 60ch; }
.results small { color: var(--mut); font-size: .85rem; }
.breach-item { background: rgba(232,115,107,0.06); border: 1px solid rgba(232,115,107,0.22); border-radius: 0; padding: .95rem 1.2rem; margin-bottom: .7rem; }
.breach-name { font-weight: 600; color: var(--ink); font-size: .98rem; margin-bottom: .2rem; }
.breach-date { color: var(--mut); font-size: .85rem; }
.fields-section { margin-top: 1.4rem; }
.fields-title { font-size: .72rem; font-weight: 500; letter-spacing: .14em; text-transform: uppercase; color: var(--mut); margin-bottom: .7rem; }
.fields-list { display: flex; flex-wrap: wrap; gap: .5rem; }
.field-tag { background: rgba(157,134,255,0.1); color: var(--violet-2); border: 1px solid rgba(157,134,255,0.28); border-radius: 0; padding: .28rem .7rem; font-size: .82rem; font-weight: 500; }
.recommendations { margin-top: 1.5rem; padding: 1.2rem 1.4rem; background: rgba(157,134,255,0.06); border: 1px solid rgba(157,134,255,0.18); color: var(--ink-2); font-size: .92rem; line-height: 1.85; }
.recommendations strong { color: var(--violet-2); display: block; margin-bottom: .4rem; font-weight: 600; }
...
.info-section p { color: var(--ink-2); font-size: 1rem; line-height: 1.7; max-width: 64ch; margin-bottom: .8rem; }
.info-section ul { list-style: none; margin: 1.1rem 0 0; padding: 0; }
...
<main class="main-wrapper" id="main">
    <div class="wrap">
        ...
        <section class="scan-hero">
            ...
            <p class="eyebrow"><?= t('breach.eyebrow') ?></p>
            <h1 class="scan-title"><?= t('breach.title') ?></h1>
            <p class="scan-desc"><?= t('breach.desc') ?></p>
        </section>
