...
try {
    ...
    if (strlen($target) > 2048) {
        throw new Exception(t('ss.url_too_long', false), 400);
    }
    ...
    // Hostname RFC-compliant: solo lettere, numeri, trattini, punti
    if (!preg_match('/^(?!-)[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,62}[a-zA-Z0-9])?(?:\.(?!-)[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,62}[a-zA-Z0-9])?)*\.[a-zA-Z]{2,}$/', $host)) {
        throw new Exception(t('ss.domain_invalid', false), 400);
    }

    // Blocca IP privati, localhost, indirizzi riservati
    $blockedPatterns = [
    ...
    foreach ($blockedPatterns as $blocked) {
        if (str_starts_with(strtolower($host), $blocked)) {
            throw new Exception(t('ss.private_addr', false), 403);
        }
    }

    // Blocca host che sono IP numerici diretti nell'URL
    if (filter_var($host, FILTER_VALIDATE_IP)) {
        throw new Exception(t('ss.ip_not_domain', false), 400);
    }

    // No porta nell'URL (deve essere 443 default)
    $port = parse_url($target, PHP_URL_PORT);
    if ($port !== null && $port !== 443) {
        throw new Exception(t('ss.port_443_only', false), 400);
    }

    // ── Verifica proprietà dominio (DNS TXT live check) ──
    ...
    // Recupera il token di verifica dal DB
    ...
    if ($domainCheck->num_rows === 0) {
        $domainCheck->close();
        $conn->close();
        throw new Exception(t('ss.domain_not_verified', false), 403);
    }
    ...
    // Ri-verifica LIVE: il record TXT deve esistere ancora nel DNS
    ...
    if ($dohResult === false) {
        $conn->close();
        throw new Exception(t('ss.dns_unreachable', false), 503);
    }
    ...
    if (!$txtFound) {
        // Il record è stato rimosso → revoca la verifica
        ...
        $revoke->close();
        $conn->close();
        throw new Exception(t('ss.txt_no_longer_found', false), 403);
    }

    $conn->close();

    // ── Verifica scanner raggiungibile ──
    ...
    if ($httpCode !== 200) {
        throw new Exception(t('ss.scanner_unavailable', false), 503);
    }

    // ── Crea sessione scansione ──
